...
# Page to Edit item
@app.route(
    '/catalog/<int:category_id>/<int:item_id>/edit', methods=['GET', 'POST']
    )
...
def gconnect():
    ...
    try:
        ...
        oauth_flow = flow_from_clientsecrets('/var/www/flaskApp/cat-a-log/client_secrets.json', scope='')
    ...
    if result['user_id'] != gplus_id:
        response = make_response(
            json.dumps("Token's user ID doesn match given id"), 401
            )
        response.headers['Content-Type'] = 'application/json'
        return response
